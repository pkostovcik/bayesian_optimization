...
class BO(object):
    def __init__(self, objective, bounds, n_iterations, n_init = 2,
        ...
        """
        Peter Kostovcik: Baayesian Optimization; diploma thesis 2017
        ...
        kernel_function     Exponential or Matern from GPy package
                            ...
                            (default = "Exponential")
        acquisition_func:   aquisition function ["UCB" == Upper Confidence Bound,
                                                 "EI" == Expected Improvement]
                                                 (default = EI)
        rand:               np.RandomState(some_number) (default = None, random choice)
        noise_var:          variance for noise (default = 0.001)
        log_info:           True/False - printing information (default = True)
    ...
    def start(self):
        ...
        if self.log_info:
            print("%s step in BO; objective value: %s at %s; time: %s s." % (1, self.y[0] ,self.x[0], time()-self.start_time1))
            print("%s step in BO; objective value: %s at %s; time: %s s." % (2, self.y[1] ,self.x[1], time()-self.start_time1))            
    ...
    def maximize(self, iterations):
        ...
        for n in np.arange(iterations):
            ...
            GPreg.optimize() # MAP selection of kernel hyper-parameters
            ...
            if self.log_info:
                print("%s step in BO; objective value: %s at %s; time: %s s." % 
                      (len(self.x), self.y[-1] ,self.x[-1], time()-self.start_time2 + self.end_time1 - self.start_time1))
            if n%10 == 0:
                self.a += 1
...
def REMBO(f, D, d, n_iterations, n_init = 2, rand = None, 
          ...
          noise_var = 0.001, log_info = True, n_iters_aqui = 15, 
    ...
    other imputs are same as for BO
    """
    if rand == None: 
